        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            <p className="text-xs font-bold uppercase tracking-widest text-gray-400 mb-3">
              Leave History This Year ({sortedReqs.length} record{sortedReqs.length !== 1 ? 's' : ''})
            </p>
            {sortedReqs.length === 0 ? (
              ...
              <div className="space-y-1.5 max-h-56 overflow-y-auto pr-1">
                {sortedReqs.map(r => {
                  ...
                  return (
                    <div key={r.id} className={`flex items-center gap-3 rounded-lg px-3 py-2 ${col.bg} border ${col.border}`}>
...
const UpcomingSchedule = ({ requests, staffList, departments, isAdmin, myDept }) => {
  ...
  /* Group by week for All / Individual views */
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 text-xs text-gray-400">
        <Clock size={12}/>
        <span>
          <strong className="text-gray-600">{visibleCount}</strong> upcoming absence{visibleCount !== 1 ? 's' : ''} — {rangeLabel}
        </span>
      </div>
            ...
            byWeek.map(([weekStart, weekReqs]) => (
              <div key={weekStart} className="card">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="ml-auto text-xs text-gray-400">{weekReqs.length} record{weekReqs.length !== 1 ? 's' : ''}</span>
                </div>
                <div className="space-y-1.5">
                  {weekReqs.map(r => renderRow(r))}
                </div>
              </div>
            ))
          )}
              ...
              <div className="overflow-x-auto">
                <table className="data-table">
                  <thead>
                    <tr>
                      <th>Name</th>
                      <th className="min-w-[200px]">Annual Leave Usage</th>
                  ...
                  <tbody>
                    {standard.map(s => {
                      ...
                      const usedPct  = s.effectiveAllowance > 0 ? Math.min(100, ((s.breakdown['Annual Leave'] || 0) / s.effectiveAllowance) * 100) : 0;
                        ...
                        >
                          ...
                          <td>
                            <div className="flex items-center justify-between text-xs mb-1">
                              <span className={`font-medium ${isNeg ? 'text-red-600' : 'text-gray-600'}`}>
                                {s.breakdown['Annual Leave'] || 0}d used
                              </span>
                              <span className={`font-semibold ${isNeg ? 'text-red-600' : isLow ? 'text-amber-600' : 'text-emerald-600'}`}>
                                {isNeg ? `${Math.abs(s.remaining ?? 0)}d over` : `${s.remaining ?? 0}d left`} / {s.effectiveAllowance}d
                              </span>
                            </div>
                            <div className="w-full bg-gray-200 rounded-full h-2.5">
                              <div
                                className={`h-2.5 rounded-full transition-all ${isNeg ? 'bg-red-500' : isLow ? 'bg-amber-400' : 'bg-emerald-500'}`}
                                style={{ width: `${usedPct}%` }}
                              />
                            </div>
                          </td>
